...
extern short tie_Screen [240][320];
/* Memory */
...
volatile int *pixel_ctrl_ptr = (int *)0xFF203020;
    ...
    for (ii=0; ii<320; ii++)
        for (jj=0; jj<240; jj++)
        *(pixelbuf + (ii<<0) + (jj<<9)) = MYIMAGE[ii][jj];
    ...
    for (i=289 ; i <320 ; i++){
    draw_line(i, 209, i, 239, 0xFFFF);   // this line is Orange
    }
   ...
   draw_line(170, 183, 180, 183, 0xFFFF); 
    ...
    for ( i=Player1_X; i < Player1_X; i ++)
        for ( j=Player1_Y; j < Player1_Y; j ++)
        plot_pixel(i,j, 0x07E0);
        
         for ( i=Player2_X; i < Player2_X; i ++)
        for ( j=Player2_Y; j < Player2_Y; j ++)
        plot_pixel(i,j, 0xEC64);
...
void updateValue_Player2(){
 //For Player 2 Bottom 
    ...
    if (((Player2_X>=30) && (Player2_X<60)) && ((Player2_Y>=1) && (Player2_Y<=30))) //Red Tile 
        ...
        Money2=Money2+3;
    else  if (((Player2_X>=270) && (Player2_X<290)) && ((Player2_Y>=1) && (Player2_Y<=30))) //Yellow Tile 
        Money2=Money2-2;
...
while(1){
   ...
   // volatile short * pixelbuf = 0xc8000000;  //Remove this
    ...
    for (ii=0; ii<320; ii++)
        for (jj=0; jj<240; jj++)
        *(pixelbuf + (ii<<0) + (jj<<9)) = tie_Screen[ii][jj];
   }
